...
@CucumberOptions(
        plugin = {"pretty", "junit:target/junit_cucumber.xml", "json:target/json-files/RunFavouritePart2Test.json", "com.epam.reportportal.cucumber.ScenarioReporter"},
        features = "classpath:com/thomsonreuters/features/favourite/part2",
        tags = {"~ ", "~@manual"},
        glue = {"com.thomsonreuters.step_definitions", "com.thomsonreuters.hooks"},
        monochrome = true,
        snippets = SnippetType.CAMELCASE)
public class RunFavouritePart2Test extends BaseCucumberTestRunner {

    private static final Logger LOG = LoggerFactory.getLogger(RunFavouritePart2Test.class);

    @BeforeClass
    public static void reporting() {
        if (System.getProperty("username","None").equals("None")) {
            User.getInstance().setUserName("PLNewAUtestuser6");
