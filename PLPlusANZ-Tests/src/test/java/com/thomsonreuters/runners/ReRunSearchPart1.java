...
@CucumberOptions(
        plugin = {"pretty", "junit:target/junit_cucumber.xml", "json:target/json-files/ReRunSearchPart1.json", "com.epam.reportportal.cucumber.ScenarioReporter"},
        features = "@target/ReRunSearchPart1.txt",
        glue = {"com.thomsonreuters.step_definitions", "com.thomsonreuters.hooks"},
        monochrome = true,
        snippets = SnippetType.CAMELCASE)
public class ReRunSearchPart1 extends BaseCucumberTestRunner {

    private static final Logger LOG = LoggerFactory.getLogger(ReRunSearchPart1.class);

    @BeforeClass
    public static void reporting() {
        if (System.getProperty("username","None").equals("None")) {
            User.getInstance().setUserName("AUtestuser2");
