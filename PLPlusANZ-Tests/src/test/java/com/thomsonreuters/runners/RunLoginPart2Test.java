...
@CucumberOptions(
        plugin = {"pretty", "junit:target/junit_cucumber.xml", "json:target/json-files/RunLoginPart2Test.json", "com.epam.reportportal.cucumber.ScenarioReporter", "rerun:target/ReRunLoginPart2.txt"},
        features = "classpath:com/thomsonreuters/features/login/part2",
        tags = {"~ ", "~@manual"},
        glue = {"com.thomsonreuters.step_definitions", "com.thomsonreuters.hooks"},
        monochrome = true,
        snippets = SnippetType.CAMELCASE)
public class RunLoginPart2Test extends BaseCucumberTestRunner {

    private static final Logger LOG = LoggerFactory.getLogger(RunLoginPart2Test.class);

    @BeforeClass
    public static void reporting() {
        if (System.getProperty("username","None").equals("None")) {
            User.getInstance().setUserName("PLAUtestuser14");
