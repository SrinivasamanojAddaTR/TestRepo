...
@CucumberOptions(
        plugin = {"pretty", "junit:target/junit_cucumber.xml", "json:target/json-files/RunLoginPart4Test.json", "com.epam.reportportal.cucumber.ScenarioReporter", "rerun:target/ReRunLoginPart4.txt"},
        features = "classpath:com/thomsonreuters/features/login/part4",
        tags = {"~ ", "~@manual"},
        glue = {"com.thomsonreuters.step_definitions", "com.thomsonreuters.hooks"},
        monochrome = true,
        snippets = SnippetType.CAMELCASE)
public class RunLoginPart4Test extends BaseCucumberTestRunner {

    private static final Logger LOG = LoggerFactory.getLogger(RunLoginPart4Test.class);

    @BeforeClass
    public static void reporting() {
        if (System.getProperty("username","None").equals("None")) {
            User.getInstance().setUserName("PLAUtestuser11");
