...
###############################################################################################################
  ...
  @gold
  Scenario Outline: [818824] Print single search result-<id>
...
###############################################################################################################
  ...
  @gold
  Scenario Outline: [818824] Download documents in zip file-<id>
    Given the user runs a free text search for the query "<query>"
    And the user selects the checkbox associated with result "1"
    And the user selects the checkbox associated with result "2"
    When user clicks on Download delivery option for Search
  ...
  Scenario Outline: [818824] Download list of items in PDF format-<id>
    Given the user runs a free text search for the query "<query>"
  ...
  Scenario Outline: [818824] Download list of items in CSV format-<id>
    Given the user runs a free text search for the query "<query>"
    And the user selects the checkbox associated with result "1"
    And the user selects the checkbox associated with result "2"
    When user clicks on Download delivery option for Search
  ...
  Scenario Outline: [818824] Download document in RTF format-<id>
    Given the user runs a free text search for the query "<query>"
  ...
  @gold
  Scenario Outline: [818824] Download document in Microsoft Word format-<id>
    Given the user runs a free text search for the query "<query>"
    And the user saves file name from following results
      | <result> |
    And the user selects the checkbox associated with result "<result>"
  ...
  Scenario Outline: [818824] Download document with table of contents in PDF format-<id>
    Given the user runs a free text search for the query "<query>"
    And the user selects the checkbox associated with result "1"
    When user clicks on Download delivery option for Search
    And the user edits the basic download options as follows
    ...
    Examples:
      | query                | name                 | docText                 | contentsText                                 | id |
      | Financial Assistance | Financial Assistance | The general prohibition | How to address financial  assistance  issues | 1  |


  Scenario Outline: [818824] Download document without table of contents in PDF format-<id>
    Given the user runs a free text search for the query "<query>"
    And the user selects the checkbox associated with result "1"
    When user clicks on Download delivery option for Search
    And the user edits the basic download options as follows
...
# The associated result has been changed from no:2 to no:3
  ...
  Scenario Outline: [818824] Email document in Microsoft Word format-<id>
    Given the user runs a free text search for the query "<query>"
    And the user selects the checkbox associated with result "6"
  ...
  @gold
  Scenario Outline: [818824][847182]Email resource link only-<id>
    Given the user runs a free text search for the query "<query>"
    And the user selects the checkbox associated with result "2"
    When user clicks on Email delivery option for Search
    And the user should be able to see Email basic tab options as follows
    ...
    Examples:
      | query           | guid                              | title           | mailbox                                           | id |
      | Share purchases | Ifb5c26cb995811e598dc8b09b4f043e0 | Share purchases | tr-anz-tester1@epam-email-pluk.thomsonreuters.com | 1  |
